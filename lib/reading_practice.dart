...
import 'reading_practice_arcade.dart';
...
class _ReadingPracticeScreenState extends State<ReadingPracticeScreen> {
  ...
  Set<String> _selectedSetKeys = {};

  List<MapEntry<String, ItemSet>> get allSets {
    List<MapEntry<String, ItemSet>> sets = [];
    
    // Add all sets that should display in reading arcade
    for (var entry in setsData.entries) {
      if (entry.value.displayInReadingArcade) {
        sets.add(entry);
      }
    }
  ...
  void _showPlayDialog(BuildContext context, {ItemSet? setToPlay}) {
          ...
          actions: [
            ...
            ElevatedButton(
              onPressed: () {
                ...
                // Use setToPlay if provided, otherwise use selection or fallback
                ItemSet? setForPractice = setToPlay;
                if (setForPractice == null) {
                  if (_isSelectionMode && _selectedSetKeys.isNotEmpty) {
                    setForPractice = setsData[_selectedSetKeys.first];
                  } else {
                    setForPractice = allSets.isNotEmpty ? allSets.first.value : null;
                  }
                }
                if (setForPractice == null) {
                  ScaffoldMessenger.of(context).showSnackBar(
                    const SnackBar(content: Text('No set selected for practice'), backgroundColor: Colors.red),
                  );
                  return;
                }
                if (roundForEveryStarred && setForPractice.items.where((item) => item.isStarred).isEmpty) {
                  ScaffoldMessenger.of(context).showSnackBar(
                    const SnackBar(content: Text('No starred items in this set'), backgroundColor: Colors.red),
                  );
                  return;
                }
                Navigator.push(
                  context,
                  MaterialPageRoute(
                    builder: (context) => ReadingPracticeArcadeScreen(
                      itemSet: setForPractice!,
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Text('Reading Practice', style: TextStyle(color: widget.isDarkMode ? Colors.white : Colors.black)),
        backgroundColor: widget.isDarkMode ? const Color(0xFF1A1A1A) : Colors.white,
        actions: [
          IconButton(
                  ...
                  onPressed: () {
                    setState(() {
                      if (_selectedSetKeys.length == allSets.length) {
                        ...
                        _selectedSetKeys = allSets.map((entry) => entry.key).toSet();
                      }
                    });
                  },
                  icon: Icon(
                        ...
                        trailing: _isSelectionMode
                            ...
                            : Row(
                                ...
                                children: [
                                  ...
                                  ElevatedButton(
                                    onPressed: () {
                                      _showPlayDialog(context, setToPlay: set);
                                    }, // Play button now shows dialog for this set
                                    style: ElevatedButton.styleFrom(
                                      backgroundColor: const Color(0xFF9A00FE),
                                      foregroundColor: Colors.white,
                                    ),
